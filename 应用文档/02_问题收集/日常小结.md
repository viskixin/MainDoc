...
<a href="../docs/02/Summary/test/CSVTest.java">CSVTest.java</a>
...
<a href="../docs/02/Summary/util/ObjectIsNullUitl.java">ObjectIsNullUitl.java</a>
...
remove
removeById / removeXXX
batchRemove

modify(推荐) / update
modifyById / modifyXXX
batchModify
...
delete
deleteById / deleteXXX
batchDelete

update
updateById / updateXXX
batchUpdate

select(获取单个对象-推荐)	|	query(获取多个对象-推荐)
selectCount(数量查询)
selectById / selectOne	 |	 queryByIds
selectList | queryXXXs
```
...
示例 → <a href="../docs/02/Summary/css/baseRef/reference.html">reference.html</a>

### @import引入css

<a href="../docs/02/Summary/css/@import/portCss.html">portCss.html</a>
...
<a href="../docs/02/Summary/JS/import/module.html">module.html</a>
...
<a href="../docs/02/Summary/JS/创建&注册&使用-自定义标签.html">创建&注册&使用-自定义标签</a>
...
1.  $(选择器).click(function(){})  →  <a href="../docs/02/Summary/JQuery/click1.html">click1</a>

```markdown
当选中的选择器被点击时触发回调函数fn()，只针对与页面已存在的选择器。
```

2.  $(选择器).on('click', function(){})  →  <a href="../docs/02/Summary/JQuery/click2.html">click2</a>
...
监听下拉列表框变化 → <a href="../docs/02/Summary/JQuery/callBack.html">callBack</a>

#### 链式调用

一次执行多个方法 → <a href="../docs/02/Summary/JQuery/chainCall.html">chainCall</a>
...
注册文件：<a href="../docs/02/Summary/exe/txtFile.reg">txtFile.reg</a>
...
设置 | 隐私和安全 | Windows 安全中心
                                 → Windows 安全中心 → 保护历史记录 → 已[阻止/隔离]威胁 → 操作 → 还原
```

![image-20250610003430902](../imgs/02/Summary/Windows/image-20250610003430902.png)

### 应用卸载

````markdown
# 如 Microsoft 365 Copilot
设置 → 应用 → 安装的应用 →  搜索 Copilot → 点击 •••  →  卸载
````
...
# C盘清理
[Win + I] → 系统 → 临时文件 → 删除文件
右击[本地磁盘(C)] → 点击[属性] → 详细信息 → 临时文件 → 删除文件
...
[Win + R] → 输入[cleanmgr]
                           ...
                           → 点击[清理系统文件] → 选择(C)盘 → 确定
                             [√] Windows 更新清理
                             [√] 传递优化文件(可选)
                             [√] 以前的Windows安装文件(升级过系统才显示，可选)
                           → 确定 → 删除文件
删除缓存文件(这些是已下载但未安装的更新包)
...
# NVIDIA Corporation
-- 已完成的更新文件(方便回滚或再次安装) → 可删除全部
C:\ProgramData\NVIDIA Corporation\NVIDIA App\UpdateFramework\ota-artifacts
-- 驱动、GeForce Experience、NVIDIA App 的完整安装包 → 可删除全部
C:\ProgramData\NVIDIA Corporation\Downloader
-- 每次安装时解压出来的驱动/组件缓存(用于 回滚 或 修复) → 只保留最新时间的文件夹，其他删除
...
# scp/rsync 可用于远程文件复制 → 服务器路径为 → 用户@主机ip:目录 或 用户@主机hostname:目录
scp 源路径文件 目标路径
rsync 源路径文件 目标路径
...
# 参数
  ...
  -z 表示压缩模式，对数据进行压缩后传输
rsync -avz 源路径文件 url
scp ./kafka-3.4.1-src.tgz root@work2:/usr/local/soft
```
