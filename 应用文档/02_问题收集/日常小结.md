...
[{bind  /宿主机(物理机)的路径 /(虚拟机)容器内的路径  rw true rprivate} {bind xxx}]
...
-- 原因
1) [redis.conf]指定的[logfile 路径] 与 [容器挂载]到[虚拟机]上面的[路径不一致]
2) [redis.conf]指定了[logfile 路径] 但 [没被容器挂载]到[虚拟机]
-- 解决
1) 修改虚拟机挂载配置
vi /虚拟机路径/redis.conf → 修改[logfile 路径]
2) 复制虚拟机目录到容器中
cd /虚拟机路径
touch 6380.log
docker cp -a /虚拟机路径/logs 容器id:/etc/redis
   或 删除容器，重新部署
docker rm -fv redis
docker run -d --name redis --restart=always -p 6379:6379 \
-v /物理机路径/redis.conf:/虚拟机路径/redis.conf \
-v /物理机路径/data:/虚拟机路径/data \
-v /物理机路径/logs:/etc/redis/logs \
-d redis:x.x.x redis-server /虚拟机路径/redis.conf \
--appendonly yes
```
