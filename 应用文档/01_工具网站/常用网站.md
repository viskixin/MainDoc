...
3) git pull --rebase →  从远程获取最新版本到本地，不会自动merge
...
| git pull origin master          | ✅          | ❌           | ✅          | 快进时呈直线；分叉时呈“分叉+合并” |
| git pull --rebase origin master | ✅          | ✅           | ❌          | 几乎总是一条直线（除非冲突中断）  |
...
A - B - C --------- M (master)
      \            /
        D - E ---
其中 M 是 merge 提交，记录了 [把 origin/master (C) 合并进来]
...
结构如下：
A - B - C - D' - E' (master)
其中 D' 和 E' 是 新的提交(hash 变了)，基于 C 重新生成：无冲突 → 内容和原来(D、E)相同；有冲突 → 线性修改(D' - E')内容
