...
```markdown
1) 假设[Repository name]为[demogh]
   在 Github [Create a new repository]
2) 进入需要上传的文件夹 → 创建[.gitignore]文件
   cd demogh → touch .gitignore
3) 把[demogh]变成[Git可管理的仓库]
   git init
4) 把[demogh目录下]的[所有文件]都[添加到仓库]
   git add .
5) 把[项目]提交到[仓库]
   git commit -m "first commit"
6) [Git仓库]关联[本地仓库]
   git remote add origin https://.../demogh.git
7) 指定[主分支]为[master] → 也可省略，直接执行步骤8
   git branch -M master
8) 把[本地库]的[所有内容]推送到[远程仓库] → 等远程仓库里面有了内容之后，可以不加-u这个参数
...
```markdown
1) 假设[Repository name]为[ghtest]
   在 Github [Create a new repository]
2) 克隆[远程仓库]到[本地文件夹]
   git clone https://.../ghtest.git
3) 进入需要上传的文件夹 → 创建[.gitignore]文件
   cd ghtest → touch .gitignore
4) 别忘记后面的. → 此操作是把[ghtest目录下]的[所有文件]都[添加进来]
   git add .
5) 版本跟踪成功之后，可用 -am 参数
   git commit -m "commit info"
6) 把[本地仓库]推送到[github]的[main分支]，首次需要输入帐号和密码
...
# Github(2020年开始)[默认分支名]为[main] → 修改步骤：
Settings | Code, planning, and automation | Repositories | Repository default branch → 输入[master] → Update
...
# 其中，HEAD@{1} 代表 reset 前的 commit (相对引用)
git reset --soft HEAD@{1}
# 或者 (绝对引用)
git reset --soft bc5edcd
# 效果
将代码库从当前状态（bc3ca04/"yyy"）强制回退到前一个提交（bc5edcd/"xxx"）
```

##### 区别

| 功能         | git log --oneline         | git reflog                        |
| ------------ | ------------------------- | --------------------------------- |
| 显示内容     | 提交历史                  | HEAD/分支引用 的 移动历史         |
| 范围         | 当前分支的 commit         | 本地所有操作记录                  |
| 能否恢复     | 否（只显示存在的 commit） | 可以恢复误操作导致的提交          |
| 是否包含操作 | 仅 commit                 | commit、reset、checkout、merge 等 |
| 是否远程同步 | 是（提交记录本身可推送）  | 否，本地记录                      |

#### * [更新分支](https://www.cnblogs.com/martinl/p/6834388.html)

```markdown
1) git fetch →  从远程获取最新版本到本地
...
-- 下面命令其实相当于 git fetch 和 git merge
git pull origin master
-- 在实际使用中，git fetch更安全一些
git fetch origin master
-- 因为在merge前，可以查看更新情况，然后再决定是否合并结束
git merge origin/master

3) git pull --rebase →  从远程获取最新版本到本地，将本地版本线性加入后面
-- 下面命令其实相当于 git fetch 和 git rebase
git pull --rebase origin master
-- fetch → 只读，不会动本地分支；rebase → 将本地分支 rebase 到远程最新，会改写本地分支的历史
git fetch origin master
-- 无论本地有没有新提交，都把本地提交逐个搬到远程最新之后，历史保持一条直线
git rebase origin master
```

| 命令                            | 是否 fetch | 是否 rebase | 是否 merge | 历史形状                              |
| ------------------------------- | ---------- | ----------- | ---------- | ------------------------------------- |
| git fetch origin master         | ✅          | ❌           | ❌          | 远端跟踪分支更新；本地分支无变化      |
| git pull origin master          | ✅          | ❌           | ✅          | 分叉+合并（除非，本地分支已经是最新） |
| git pull --rebase origin master | ✅          | ✅           | ❌          | 一条直线（内容冲突时，线性修改文件）  |

```markdown
# 提交树画
远端 origin/master 最新有 3 个提交：A - B - C
本地 master 分支基于 B 做了两个提交：D - E
当前结构如下：
远端: A - B - C (origin/master)
本地:       \
              D - E (master)

# git pull origin master （fetch + merge）
执行后，Git 会把 origin/master 更新到 C，然后做一次 merge commit
结构如下：
A - B - C --------- M (master)
      \            /
        D - E ---
其中 M 是 merge(与内容冲突无关，与分支变动有关) 提交，记录了 [把 origin/master (C) 合并进来]
特点：保留了分叉历史，看得出团队协作痕迹

# git pull --rebase origin master （fetch + rebase）
执行后，Git 会把本地的 D - E 重新“移动”到 C 之后，形成一条直线
结构如下：
A - B - C - D' - E' (master)
其中 D' 和 E' 是 新的提交(hash 变了)，基于 C 重新生成
  与 C 内容 无冲突 → (D'、E')与(D、E)内容相同
  与 C 内容 有冲突 → 线性修改(D' - E')与(D、E)内容不同
特点：历史更干净，没有 merge 节点，但会改写本地提交
```
...
1) 工作区（Working Directory）
   电脑上实际看到的项目目录（包含 .git 文件夹的目录）
2) 暂存区（Staging Area / Index）
   一个临时区域（位于 .git/index），用于标记下次提交的更改
   关键命令：
     git add <file>：将文件从工作区添加到暂存区
     git reset <file>：将文件从暂存区移回工作区（取消暂存）
3) 本地版本库（Local Repository）
   Git 的本地数据库（.git 目录），存储项目的所有历史版本（提交记录）
   关键命令：
     git commit -m "message"：将暂存区的内容永久保存到本地版本库
     git log：查看提交历史
...
```bash
# 测试 http 代理
curl -x http://127.0.0.1:7890 https://www.google.com -k -v
# 测试 socks5 代理
curl -x socks5h://127.0.0.1:7891 https://www.google.com -k -v
# Git 用 OpenSSL 而不是 SChannel
...
-- 解决方案1 → 设置全局代理(http) → 不加 --global，只对当前仓库生效
git config --global http.proxy http://127.0.0.1:7890
git config --global https.proxy http://127.0.0.1:7890
-- 解决方案2 → 设置全局代理(socks5) → 不加 --global，只对当前仓库生效
git config --global http.proxy socks5h://127.0.0.1:7891
git config --global https.proxy socks5h://127.0.0.1:7891
-- 验证是否生效
git config --global --get https.proxy
git ls-remote https://github.com/viskixin/xxx.git
-- 全局取消 → 不加 --global，单个仓库取消
git config --global --unset http.proxy
-- 注意
设置 https 代理就行，不行再设置 http
```

#### TLS connect error

```markdown
# fatal: unable to access 'https://gitee.com/...': TLS connect error: error:xxx
-- 原因
Git 无法与 Gitee 建立安全 TLS 连接
-- 解决方案 → 临时关闭代理
git config --global --unset http.proxy
-- 注意
关闭 http 即可
...
-- 解决方案
   ...
   3) 如果以上两个方法不起作用，那么采用以下方法：
      进入 控制面板 → 用户账户 → 管理你的凭据 → windows凭据 → 普通凭据
...
## 【[TortoiseSVN](https://tortoisesvn.net)】官网
...
## 【[Oracle](https://www.oracle.com/index.html)】官网
...
![image-20210306204644184](../imgs/01/Website/Centos/image-20210306204644184.png)
...
### # [MinGW 网址](https://www.mingw-w64.org)
...
#### [vue](https://www.npmjs.com/package/vue?activeTab=versions)
...
#### [sass](https://github.com/sass/node-sass/tags) | [sass-loader](https://github.com/webpack-contrib/sass-loader/tags)
...
# --save-dev 表示将包添加到 package.json 文件中的 devDependencies 字段。这意味着这个包仅在开发环境中使用，例如用于构建、测试或代码检查等任务，而在生产环境中不需要
...
## 【[layer](https://layer.layui.com)】弹层组件
...
```markdown
# 选择免费计划
点击左上角[CLOUDFLARE(图标)] | Zero Trust | Settings | Account → Plan → Free → 输入团队名称(viski) → Proceed to payment
...
```markdown
# 修改账户名
Account Home | 账户名右侧[┇] → Change account name
或者
Manage Account | Configurations | Preferences | Account name → Change name

# 修改域名
Workers & Pages | Subdomain → 输入[新子域名] → Continue
```

### [Workers部署](https://z008.blogspot.com/2024/03/2CJD.html)

<a href="../docs/01/Website/VPN/_worker.js">_worker.js</a>  →  [GitHub资源](https://github.com/3Kmfi6HP/EDtunnel)

编辑服务器  |  <a href="../docs/01/Website/VPN/IP_域名_端口.md">节点优选</a>

```markdown
# vless copy 地址
https://6kw.weski.dpdns.org/6d68f9e6-07f1-4d5e-856d-a0b61cf09d14

# vlessMain
vless://6d68f9e6-07f1-4d5e-856d-a0b61cf09d14@168.138.46.67:443?encryption=none&security=tls&sni=6kw.weski.dpdns.org&fp=randomized&type=ws&host=6kw.weski.dpdns.org&path=%2F%3Fed%3D2048#6kmfi_Japan_D
```

### [Pages部署](https://z008.blogspot.com/2024/03/YJJD.html)

<a href="../docs/01/Website/VPN/worker.zip">worker.zip</a>  →  [GitHub资源](https://github.com/ZZY202203/Cloudflare-Vless1)
...
[变量介绍](https://www.youtube.com/watch?v=s91zjpw3-P8) | [订阅器-使用](https://www.youtube.com/watch?v=OjqCKeEY7DQ) → <a href="../docs/01/Website/VPN/订阅器使用.txt">文档</a> | [订阅器-部署](https://www.youtube.com/watch?v=AtCF7eq0hcE) | [订阅器-讲解](https://www.youtube.com/watch?v=p-KhFJAC4WQ)

```markdown
# 说明
这是一个基于 CF Worker 平台的脚本，在原版的基础上修改了显示 VLESS 配置信息转换为订阅内容
使用该脚本，可以方便地将 VLESS 配置信息使用在线配置转换到 Clash 或 Singbox 等工具中
...
使用该脚本，可以方便地将 Trojan 配置信息使用在线配置转换到 Clash 或 Singbox 等工具中
```
...
<a href="../docs/01/Website/VPN/vless_worker.zip">vless_worker.zip</a>  →  [Pages 上传](https://www.youtube.com/watch?v=McdRoLZeTqg)  (将  _worker.js  压缩成  vless_worker.zip)

[_worker.js](https://github.com/yonggekkk/Cloudflare_vless_trojan/blob/main/Trojan_workers_pages/_worker.js)  |  <a href="../docs/01/Website/VPN/trojan_worker.zip">trojan_worker.zip</a>  →  [Trojan 部署](https://www.youtube.com/watch?v=lmhhL8M1k0I)
...
# 代理软件客户端
https://github.com/2dust/v2rayN/releases/tag/7.12.6
https://github.com/2dust/v2rayN/releases/tag/7.10.5

# 下载
v2rayN-windows-64-With-Core.zip | v2rayN-windows-64-SelfContained-With-Core.7z | 
...
设置 → 参数设置 → v2rayN 设置 → 开机启动
               ...
               → Core类型设置 → Shadowsocks[sing_box] → VLESS[Xray] → Trojan[Xray]
...
```bash
# 注意
所有Cloudflare项目 都转移到了 linhanxavi@gmail.com

# Author(6Kmfi6HP)	DNS(US.KG & DynV6)		Email(linhanxavi@gmail.com)
