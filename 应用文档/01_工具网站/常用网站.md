...
# --mount source=宿主机(Docker Engine 中的默认存储路径)/oracle_vol,target=虚拟机容器内路径
将(虚拟机)容器内 /home/oracle/app/oracle/oradata oracle_11g 映射到(宿主机) /var/lib/docker/volumes/oracle_vol
...
# 单节点
-- 不设密码
docker run --name redis5 --restart=always -p 6379:6379 \
...
-d redis5:5.0.7 redis-server /etc/redis/redis_6379.conf \
--appendonly yes
-- 设置密码
docker run --name redis5 --privileged=true --restart=always -p 6379:6379 \
-v /docker-software/redis_v5/redis_6379.conf:/etc/redis/redis_6379.conf \
-v /docker-software/redis_v5/data/d_6379:/etc/redis/data1 \
-v /docker-software/redis_v5/logs:/etc/redis/logs \
-d redis5:5.0.7 redis-server /etc/redis/redis_6379.conf \
...
# 主从快速搭建
1) 启动 Redis 主节点
docker run -d --name redis-master -p 6379:6379 redis:latest --requirepass 123456 --port 6379
2) 启动 Redis 从节点1
...
```markdown
# 无法科学上网
设置 → 参数设置 → Core: 基础设置 → 启用分片 (Fragment)
              → Core类型设置 → Shadowsocks[sing box]
```
...
<a href="..\document\01\Website\VPN\Clash介绍.md"># Clash介绍</a>
